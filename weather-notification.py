...
try:
    ...
    if "results" in geo_res:
        ...
        if "current_weather" in weather_res:
            ...
            notification.notify(title = "weather update", message = weather_info,timeout = 6 )

        else:
            print("weather data not found")
    else:
        print(f"the {city} is not found. please check the spelling.")
        notification.notify(title='Wrong City', message = f'{city} not Found', timeout= 5)
